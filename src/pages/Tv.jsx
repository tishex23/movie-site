...
export default function Tv() {
  ...
  useEffect(() => {
    const getData = async () => {
      try{
        const response = await fetch(`https://api.themoviedb.org/3/tv/popular?api_key=db5079a3779189dc15eb5636dc6d3fac&page=${currentPage}`);
  ...
  return (
    <div>
      <div className="m-28 flex flex-wrap justify-start gap-2">
        {data.map(movie => <Link key={movie.id} to={`tv/${movie.id}`}> <Card title={movie.original_title} source={movie.poster_path} id={movie.id}  /> </Link>)}
      </div>
